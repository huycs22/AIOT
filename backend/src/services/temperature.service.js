...
const Threshold = require('../models/temperatureBound.model');
const Record = require('../models/temperatureRecord.model');

// Fetch from Adafruit IO
async function fetchSensorData(limit = 10) {
  const resp = await adafruit.get(`/feeds/temperature/data?limit=${limit}`);
...
async function getBound() {
  ...
  if (!t) {
    ...
    await adafruit.post(`/feeds/temperature-threshold/data`, {
