...
const Threshold = require('../models/brightnessBound.model');
const Record = require('../models/brightnessRecord.model');

// Fetch from Adafruit IO
async function fetchSensorData(limit = 10) {
  const resp = await adafruit.get(`/feeds/brightness/data?limit=${limit}`);
...
async function getBound() {
  ...
  if (!t) {
    ...
    await adafruit.post(`/feeds/brightness-threshold/data`, {
