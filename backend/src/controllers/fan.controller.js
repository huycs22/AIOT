const { controlModeSchema, recordSchema } = require('../validators/fan.validator');
const svc = require('../services/fan.service');

// POST /api/fan/control  → send intensity
exports.setValue = async (req, res, next) => {
  try {
    console.log(req.body)
    const { value } = await recordSchema.validateAsync(req.body);
    const result = await svc.setfanValue(value);
